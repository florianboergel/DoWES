...
RotorDiameter = 0.4705 % Electrical conversion efficiency
...
% Calcualte Twist-Angle and Chordlength
    ...
    for BladeElementRadius = BladeElementRadii
        alpha_li    =   atan(2.0/3.0*R/(BladeElementRadius*lambdad))
        alpha_twisti=   alpha_li - alpha_Ad
        ...
        chord_i = 2*pi*R/N*8/9/cld*1/(lambdad*sqrt((lambdad*BladeElementRadius/R)^2+4/9))
        dP2(count) = N * 0.5 *rho*(sqrt(Ud^2 + RotationalSpeed^2))*chord_i*cld*sin(alpha_li)*RotationalSpeed*BladeElementRadius^2
        dP(count) = 16/27*rho*0.5*Ud^3*(2*pi*BladeElementRadius)
        chord(count) = chord_i
        count = count +1;
    end
%%
